...
// Create axios instance with localhost
...
export const setAxiosToken = (token) => {
  console.log("🔧 setAxiosToken called with:", token ? "token present" : "no token");
  currentToken = token;
  
  // Also set default header for all future requests
  if (token) {
...
axiosClient.interceptors.request.use(
  (config) => {
    ...
    const token = currentToken || axiosClient.defaults.headers.common['Authorization']?.replace('Bearer ', '');
    ...
    console.log("  - Default header:", axiosClient.defaults.headers.common['Authorization'] ? "present" : "missing");
    ...
    console.log("  - Final Auth Header:", config.headers.Authorization || "undefined");
...
axiosClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (!error.response) {
      console.log("❌ NETWORK ERROR - Backend server not reachable at:", axiosClient.defaults.baseURL);
      console.log("  - Check if backend server is running");
      console.log("  - Try accessing in browser:", `${axiosClient.defaults.baseURL}/`);
      return Promise.reject(error);
    }
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        ...
        // Try to refresh token - using localhost
...
// Helper function to get full image URLs - changed to localhost
