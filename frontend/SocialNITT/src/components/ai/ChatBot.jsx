...
function Chatbot() {
  const [open, setOpen] = useState(false);
  const [messages, setMessages] = useState([
    { from: "bot", text: "Hi! Ask me anything about SocialNITT." }
  ]);
  const [input, setInput] = useState("");
  const [loading, setLoading] = useState(false);
  const panelRef = useRef(null);
  ...
  const sendMessage = async (e) => {
    ...
    try {
      // Send user message to Rasa REST endpoint
      const res = await axios.post(
        "http://localhost:5005/webhooks/rest/webhook",
        {
          sender: "user1", // You can use a unique user ID if you have auth
          message: input
        }
      );
      // Rasa may return multiple messages
      const botReplies = res.data.length
        ? res.data.map((msg) => msg.text).join("\n")
        : "Sorry, I didn't understand that.";
      setMessages((msgs) => [...msgs, { from: "bot", text: botReplies }]);
    } catch (err) {
      setMessages((msgs) => [
        ...msgs,
        { from: "bot", text: "Sorry, there was an error. Try again later." }
